...
public class MainActivity extends AppCompatActivity implements ThumbnailsRecyclerView.OnItemSelectedListener {
    ...
    private final ActivityResultLauncher<Uri> takePhotoLauncher = registerForActivityResult(
            new ActivityResultContracts.TakePicture(), isPhotoTaken -> {
                if (isPhotoTaken) {
            ...
            LicenseManager.initLicense(
                    ...
                    (isSuccess, error) -> {
                        if (!isSuccess) {
                            ...
                            runOnUiThread(() -> binding.tvLicenseError
    ...
    private void decodeSelectedUri(Uri uri) {
        ...
        mDecodeThreadExecutor.submit(() -> {
            ...
            // The method returns a CapturedResult object that contains an array of
            // CapturedResultItems.
            // CapturedResultItem is the basic unit from which you can get the basic info of
    ...
    private void initView() {
        ...
        binding.btnTakePhotoCenter.setOnClickListener(v -> ActivityCompat.requestPermissions(this,
                new String[] { android.Manifest.permission.CAMERA }, REQUEST_CAMERA_CODE));
        binding.btnTakePhotoTop.setOnClickListener(v -> ActivityCompat.requestPermissions(this,
                new String[] { android.Manifest.permission.CAMERA }, REQUEST_CAMERA_CODE));
    ...
    private void takePhoto() {
        String cameraPath = Objects.requireNonNull(getExternalCacheDir()).getAbsolutePath() + "/Dysnamsoft/"
    ...
    private void showBarcodeResult(CapturedResult result, Uri uri) {
        ...
        if (result.getErrorCode() != 0) {
            new AlertDialog.Builder(this)
                    .setMessage(String.format(Locale.getDefault(), "ErrorCode: %d %nErrorMessage: %s",
